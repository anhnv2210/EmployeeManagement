...
import { getTaiSanById, updateTaiSan } from '@/services/TaiSanService';
import { formatISO } from 'date-fns';
import React, { useEffect, useState } from 'react'
import { useNavigate, useParams } from 'react-router-dom';

const SuaTaiSanComponent = () => {
    const { id } = useParams();
    ...
    useEffect(() => {
        getTaiSanById(id).then(response => {
            setTaiSan(response.data);
        }).catch(error => {
            console.error(error);
        });
    ...
    const handleSubmit = (e) => {
        ...
        updateTaiSan(id, updatedTaiSan).then(() => {
            navigate('/tai-san');
        }).catch(error => {
            console.error("Error updating tài sản:", error.response ? error.response.data : error.message);
        });
    }
    ...
    return (
        <div className="container mx-auto mt-5" style={{ minHeight:'calc(100vh - 130px)' }}>
            <div className="card p-4 mt-4 mb-3 w-1/2 mx-auto shadow-lg rounded-lg">
                <h2 className="text-3xl font-bold text-center mb-4">Sửa Tài sản cấp phát</h2>
...
export default SuaTaiSanComponent