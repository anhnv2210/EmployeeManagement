...
const ListTaiSanComponent = () => {
    ...
    useEffect(() => {
        const fetchTaiSans = () => {
            listTaiSan(statusFilter, pageSize, currentPage)
                .then((response) => {
    ...
    return (
        <div className="container mx-auto mt-5" style={{ minHeight: 'calc(100vh - 130px)' }}>
            ...
            <table className="min-w-full bg-white border border-gray-200 rounded-lg overflow-hidden">
                ...
                <tbody>
                    {taiSans.length>0 ? taiSans.map(taiSan => (
                        <tr key={taiSan.id}>
                            <td className="text-center py-2 px-4 border-b-2">{taiSan.id}</td>
                            ...
                            <td className="text-center py-2 px-4 border-b">{taiSan.isActive ? "Áp dụng" : "Ngưng áp dụng"}</td>
