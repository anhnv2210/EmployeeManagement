...
const ThemPhongBanComponent = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const exists = await checkTenPhongBanExists(phongBan.tenPhongBan);
            if (exists) {
                setError('Tên phòng ban đã tồn tại. Vui lòng chọn tên khác.');
                return;
            }
            await addPhongBan(phongBan);
            navigate('/phong-ban');
        } catch (error) {
            console.error("Error adding PhongBan:", error.response ? error.response.data : error.message);
            setError('Có lỗi xảy ra khi thêm phòng ban.');
        }
    };
  ...
  return (
    <div className="container mx-auto">
            <div className="card bg-white shadow-md p-6 mt-6 mb-6 mx-auto w-full md:w-1/2 lg:w-1/3 rounded-lg">
                <h2 className="text-3xl font-bold text-center mb-6">Thêm mới phòng ban</h2>
                <form onSubmit={handleSubmit}>
                    {error && <div className="bg-red-100 text-red-700 p-4 rounded mb-4">{error}</div>}
                    <div className="mb-4">
                        <label className="block text-gray-700 font-bold mb-2">Tên phòng ban</label>
                        <input 
                            type="text" 
...
export default ThemPhongBanComponent